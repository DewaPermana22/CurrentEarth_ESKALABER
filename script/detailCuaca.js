...
const elCuacaProv = async () => {
  ...
  prov.forEach(async (p) => {
    if(p.name == "DKI JAKARTA") {
      p.name = "JAKARTA";
    } else if(p.name == "DI YOGYAKARTA") {
      p.name = "YOGYAKARTA";
    }

    const weatherApiFetch = fetch(`https://api.openweathermap.org/data/2.5/weather?q=${p.name},id&appid=${keyWeather}&lang=id`)
    .then(res => {
      if(!res.ok) {
        throw new Error('Error Fetching Data Weather API');
      }

      return res.json();
    })
    .then(async (weatherApi) => {
